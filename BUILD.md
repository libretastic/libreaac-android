...
The following builds the complete unsigned LibreAAC 0.1.12 APK from a clean
...
git clone --branch v0.1.12 --recurse-submodules \
  https://github.com/libretastic/libreaac-android.git

test "$(git -C libreaac-android describe --tags --exact-match)" = "v0.1.12"
...
For the latest development revision, omit `--branch v0.1.12` and the exact-tag
...
glab release list
glab release view
glab release view v0.1.12
glab release download v0.1.12 -n "libreaac-v0.1.12.*"
glab release upload v0.1.12 ./additional-file
```
